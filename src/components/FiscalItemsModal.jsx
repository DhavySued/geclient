...
import { REGIMES } from '../context/FiscalConfigContext'
...
export default function FiscalItemsModal({ items, onAdd, onRemove, onUpdate, onReorder, onClose }) {
  ...
  function toggleRegime(item, regime) {
    ...
    if (next.length === 0 || next.length === REGIMES.length) next = ['all']
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl border border-gray-200 shadow-2xl w-full max-w-2xl flex flex-col max-h-[85vh]">

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-gray-900">Itens de Consulta Fiscal</h2>
        ...
        <div className="px-5 py-4 border-t border-gray-100 flex-shrink-0">
          <form onSubmit={handleAddItem} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand-500/50"
            />
            <button
              type="submit"
              disabled={!newLabel.trim()}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-brand-500 hover:bg-brand-400 text-gray-900 text-sm font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            >
              <Plus size={15} />
              Adicionar
            </button>
  ...
  return (
    <div className={`rounded-xl border transition-all ${
      item.active
        ...
        : 'bg-gray-100/20 border-gray-100 opacity-50'
    }`}>
      ...
      <div className="flex items-center gap-2 p-3">
        ...
        <div className="flex flex-col gap-0.5 flex-shrink-0 w-7">
          <button
            ...
            disabled={idx === 0}
            className="p-0.5 rounded text-gray-600 hover:text-gray-600 disabled:opacity-20 disabled:cursor-not-allowed"
          >
            <ChevronUp size={13} />
          </button>
          <button
        ...
        <button
          ...
          className={`w-4 h-4 rounded flex-shrink-0 border-2 transition-all ${
            item.active ? 'bg-brand-500 border-brand-500' : 'bg-transparent border-gray-600'
          }`}
        ...
        {editingLabel ? (
          <input
            ...
            className="flex-1 bg-white border border-brand-500/50 rounded-lg px-2 py-1 text-sm text-gray-900 focus:outline-none"
          />
        ) : (
          <span
            className="flex-1 text-sm text-gray-600 cursor-pointer hover:text-gray-900"
        ...
        <input
          ...
          className="w-14 flex-shrink-0 bg-white border border-gray-200 rounded-lg px-2 py-1 text-sm text-center text-brand-300 focus:outline-none focus:border-brand-500/50"
          title="Peso (1–100)"
        />
      ...
      {/* Applicability row */}
      <div className="px-3 pb-2.5 pt-2 flex flex-wrap gap-3 items-center border-t border-gray-200/30">
        {/* Regime chips */}
    ...
    <button
      ...
      className={`px-2 py-0.5 rounded-full text-[10px] font-medium border transition-all ${
        active
          ...
          : 'bg-transparent border-gray-200 text-gray-600 hover:border-gray-400 hover:text-gray-400'
      }`}
    >
      {label}
    </button>
