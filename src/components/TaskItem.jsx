...
export const PRIORITY_STYLE = {
  ...
  nenhuma: 'bg-gray-100 text-gray-500 border-gray-300',
}
...
function EditForm({ task, users, clients, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSave} className="p-4 rounded-xl border border-brand-500/30 bg-gray-100/90 space-y-3">
      <input
        autoFocus
        value={title}
      ...
      <div className="space-y-2">
        ...
        <label className="flex items-center gap-2 cursor-pointer select-none">
          <div
            ...
            className={`w-8 h-4 rounded-full transition-colors flex-shrink-0 relative ${inAgenda ? 'bg-brand-500' : 'bg-gray-700'}`}
          >
          ...
          <div className="flex items-center gap-1.5">
            <CalendarDays size={12} className={inAgenda ? 'text-brand-400' : 'text-gray-600'} />
            <span className={`text-xs ${inAgenda ? 'text-gray-600' : 'text-gray-600'}`}>
      ...
      <div className="flex gap-2 pt-1">
        <button
          type="submit"
          className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-brand-500 hover:bg-brand-400 text-gray-900 text-xs font-semibold transition-all"
        >
        ...
        <button
          type="button"
          onClick={onCancel}
          className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-gray-200 text-gray-400 hover:text-gray-700 text-xs transition-all"
        >
          <X size={13} /> Cancelar
        </button>
      </div>
...
function TemplateCard({ task, users, clients, onUpdate, onDelete, canEdit = true, canDelete = true }) {
...
export default function TaskItem({ task, users = [], clients = null, onToggle, onUpdate, onDelete, onOpenClient, canEdit = true, canDelete = true }) {
  const [editing, setEditing] = useState(false)
    ...
    <div className={`group flex items-start gap-3 p-4 rounded-xl border transition-all ${
      ...
      <button
        ...
        className={`mt-0.5 flex-shrink-0 transition-colors ${
          isDone ? 'text-emerald-400' : 'text-gray-600 hover:text-brand-400'
      ...
      {/* Content */}
      <div className="flex-1 min-w-0">
        <p className={`text-sm mb-1 ${isDone ? 'line-through text-gray-400 font-normal' : 'text-gray-900 font-semibold'}`}>
          {task.title}
        </p>

        {task.description && (
          <div
            className="rich-content text-xs text-gray-500 mb-2 leading-relaxed"
